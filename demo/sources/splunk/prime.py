...
Token handoff. The Sources form authenticates Splunk with an API TOKEN (there is no username field
for a splunk source), and Splunk generates the token value — it cannot be chosen in advance. So
this script enables token auth, mints an admin token, and writes it to ``.splunk-demo-token``
beside this file. That file is the ONE handoff point: start-ui-install.sh exports it as
``PROVISA_DEMO_SPLUNK_TOKEN`` for fragment.yaml's ``${env:PROVISA_DEMO_SPLUNK_TOKEN}``, and the
source-to-query e2e reads the same file to type into the form. No default and no fallback: an
unset variable fails the config load loudly (EnvSecretsProvider raises).
"""
...
TOKEN_FILE = Path(__file__).resolve().parent / ".splunk-demo-token"
...
def _mint_api_token(client: httpx.Client) -> str:
    """Enable Splunk's token authentication and mint an admin token (the value is generated by
    Splunk, so it is read back from the create response)."""
    r = client.post(
        f"{BASE}/services/admin/token-auth/tokens_auth",
        params={"output_mode": "json"},
        data={"disabled": "false"},
        auth=AUTH,
    )
    if r.status_code not in (200, 201):
        raise RuntimeError(f"enabling token auth failed: {r.status_code} {r.text}")
    # Splunk stores issued tokens in its KVStore, which finishes starting well AFTER the
    # management API begins answering: minting before then fails 503 "KVStore is not ready. Token
    # auth system will not work." (observed live on a cold container whose REST API had been
    # serving for ~30s). Wait for the KVStore to report ready, then mint.
    deadline = time.monotonic() + 300
    while True:
        r = client.get(f"{BASE}/services/kvstore/status", params={"output_mode": "json"}, auth=AUTH)
        status = ""
        if r.status_code == 200:
            status = r.json()["entry"][0]["content"]["current"].get("status", "")
        if status == "ready":
            break
        if time.monotonic() > deadline:
            raise RuntimeError(f"splunk KVStore never became ready (last status: {status!r})")
        time.sleep(5)
    ...
    r = client.post(
        ...
        data={
            "name": USER,
            "audience": "provisa-demo",
            "expires_on": "+30d",
        },
        auth=AUTH,
    )
    if r.status_code not in (200, 201):
        raise RuntimeError(f"minting a Splunk API token failed: {r.status_code} {r.text}")
    return r.json()["entry"][0]["content"]["token"]
