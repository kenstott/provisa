...
2. **Применение связей**: обходы должны существовать в утверждённом каталоге связей, если роль не обладает `ignore_relationships` — среди предустановленных системных ролей ею обладает только `modeler` (REQ-001, REQ-1297). В режиме высокой безопасности эта возможность игнорируется, и ни один обход не выходит за пределы каталога (REQ-693)
