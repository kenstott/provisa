...
Для **межисточниковых связей** это происходит автоматически при старте: каждая связь с `materialize: true`, ноги которой попадают более чем в один источник, генерирует MV `JoinPattern` (`auto-mv-<rel_id>`). (REQ-158) Отдельная конфигурация MV не требуется. Когда компилятор видит этот join в запросе, переписчик прозрачно подставляет предварительно материализованный результат. Связи в пределах одного источника не генерируют ничего — такие JOIN и так быстры за счёт прямого выполнения. (REQ-159) [tool-verified: `provisa/api/app_loaders.py`]

**Связь через junction** материализует свой обход, а не прямое соединение: ассоциативная таблица — это третья нога, поэтому шаблон несёт переход от источника, переход через junction и дискриминатор, закрепляющий набор строк за одним типом ребра, а собственные столбцы junction попадают в представление рядом со столбцами цели. (REQ-1586) Поскольку junction считается ногой, ребро, junction которого лежит в источнике, отличном от двух связываемых таблиц, является межисточниковым, даже если эти две таблицы совпадают по источнику. Переписчик сопоставляет два перехода как цепочку — второй обязан начинаться с псевдонима, введённого первым, — поэтому запрос, достигающий тех же двух таблиц в обход junction, читает базовые таблицы, а представление, построенное для одного значения дискриминатора, никогда не отвечает на обход, отфильтрованный по другому.
