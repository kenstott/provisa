...
Поле `password` стартового пакета несёт учётные данные, и то, *чем* они являются, определяет метод: персональный токен доступа, bearer-токен OIDC или пароль для настроенного провайдера. При провайдере `basic` с `auth.scram: true` пароль доказывается через SCRAM-SHA-256, а не передаётся. Клиентские сертификаты поддерживаются. В режиме доверия (`none`) имя пользователя напрямую отображается в роль, а пароль игнорируется.

Полная таблица «интерфейс × метод» приведена в [Модели безопасности](security.md#surfaces-and-credentials). MD5 не поддерживается; включите TLS (`PROVISA_PGWIRE_CERT` / `PROVISA_PGWIRE_KEY`) при работе через недоверенную сеть.
...
Flight несёт свои учётные данные в JSON-полезной нагрузке, в поле `token` — это bearer-токен провайдера или персональный токен доступа. Их принимают и рукопожатие, и каждый билет, и оба проверяют их одинаково, поэтому клиент, прошедший аутентификацию при рукопожатии, всё равно предъявляет токен при каждом `do_get`. Соседнее поле `role` *запрашивает* роль; сервер выводит допустимые роли этой личности и подставляет разрешённое значение, так что строка роли в билете никогда не является личностью. (REQ-1263) См. [Модель безопасности](security.md#surfaces-and-credentials).
...
Каждый RPC должен нести учётные данные в ключе метаданных `authorization` — токен провайдера или персональный токен доступа. `x-provisa-role` запрашивает роль из допустимого набора этой личности; это не учётные данные и никогда ими не было. Клиентские сертификаты поддерживаются. См. [Модель безопасности](security.md#surfaces-and-credentials).

Потоковые запросы выдают одно сообщение на строку; мутации унарны.
