...
- Связи нельзя записывать — они выводятся из соединений, объявленных в семантическом слое, а не хранятся как рёбра. Обращение к связи как к цели — жёсткая ошибка. (REQ-665) Ребро через junction не исключение: стоящая за ним ассоциативная таблица сама является зарегистрированной таблицей, и строки пишутся именно в неё, а не в ребро. (REQ-1586)
...
2. **Свойства связи есть только у рёбер через junction.** Ребро, объявленное на паре столбцов внешнего ключа, существует исключительно как метаданные соединения в семантическом слое (REQ-574) и не несёт хранимых атрибутов, поэтому `WHERE r.since > 2020` или `RETURN r.weight` не имеют для него смысла. Ребро, объявленное на junction-таблице, их несёт: остальные столбцы ассоциативной таблицы — это свойства связи, `RETURN r` их возвращает, а `WHERE` по одному из них компилируется в предикат на псевдониме junction — то есть ограничивает обход, а не фильтрует уже собранные строки. (REQ-1586) Сама junction-таблица исчезает с узловой стороны схемы графа: здесь она ребро, а везде остальном — таблица.
