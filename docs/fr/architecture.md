...
Pour les **relations inter-sources**, cela se produit automatiquement au démarrage : chaque relation portant `materialize: true` dont les pattes tombent dans plus d'une source génère une MV `JoinPattern` (`auto-mv-<rel_id>`). (REQ-158) Aucune config de MV séparée n'est requise. Quand le compilateur voit cette jointure dans une requête, le réécrivain substitue le résultat pré-matérialisé de manière transparente. Les relations à l'intérieur d'une même source ne génèrent rien — ces JOIN sont déjà rapides via l'exécution directe. (REQ-159) [tool-verified: `provisa/api/app_loaders.py`]

Une **relation adossée à une jonction** matérialise son parcours plutôt qu'une jointure directe : la table associative est une troisième patte, si bien que le motif porte le saut source, le saut de jonction et le discriminant qui fige l'ensemble de lignes sur un seul type d'arête, les colonnes propres de la jonction atterrissant dans la vue à côté de celles de la cible. (REQ-1586) Parce que la jonction compte comme une patte, une arête dont la jonction se trouve dans une source différente de celle des deux tables qu'elle relie est inter-sources même quand ces deux-là concordent. Le réécrivain apparie les deux sauts comme une chaîne — le second doit partir de l'alias introduit par le premier — de sorte qu'une requête atteignant ces deux mêmes tables sans passer par la jonction lit les tables de base, et qu'une vue construite pour une valeur de discriminant ne répond jamais à un parcours filtré sur une autre.
...
Un rôle nomme au plus un parent via `parent_role_id`. (REQ-215) Lors de la construction à l'exécution, `security/inheritance.py` parcourt la chaîne de chaque rôle de l'enfant vers le haut et la replie dans les propres copies des données chargées pour cette construction, de sorte que chaque recherche ultérieure reste indexée par l'id du rôle agissant. (REQ-1677) Les capacités et l'accès aux domaines sont l'union de la chaîne. Le `visible_to`, `writable_by` et `unmasked_to` d'une colonne, ainsi que le `visible_to` d'une métrique, d'une fonction ou d'un webhook, s'appliquent à un rôle lorsqu'ils le nomment lui ou un ancêtre. La RLS se résout par table avec priorité à l'enfant : le rôle le plus proche dans la chaîne qui possède une règle pour la table — sa propre règle de table avant sa règle de domaine — fournit l'unique prédicat exécuté ; la règle d'un ancêtre n'est jamais combinée avec elle. Un rôle hérite de la limite de débit de son parent lorsqu'il n'en définit aucune. Un parent doit exister, ne peut pas être le rôle lui-même et ne peut pas fermer un cycle ; un rôle dont d'autres rôles héritent ne peut pas être supprimé tant qu'ils n'ont pas été rattachés à un autre parent. (REQ-1677)
