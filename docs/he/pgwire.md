...
כל ערך provider אחר מחזיר שגיאת FATAL בהתחברות. (REQ-529) הפרוטוקול תמיד משתמש בסוג אימות PG 3 (סיסמת cleartext). (REQ-529) אל תשתמשו במצב trust על חיבור לא-מוצפן. [tool-verified: `server.py:282-311`]
