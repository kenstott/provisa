...
Para **relaciones entre orígenes**, esto sucede automáticamente en el arranque: cada relación entre orígenes aprobada genera una MV `JoinPattern` (`auto-mv-<rel_id>`). (REQ-158) No se requiere configuración de MV independiente. Cuando el compilador detecta esa unión en una consulta, el reescritor sustituye el resultado premateializado de forma transparente.

Para **relaciones del mismo origen**, los stewards pueden optar explícitamente mediante `materialize: true`. Los JOIN del mismo origen ya son rápidos mediante ejecución directa, por lo que la materialización solo vale la pena para rutas de unión muy activas. (REQ-159)
