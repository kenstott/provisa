...
- No se pueden escribir relaciones — se derivan de uniones de clave foránea, no se almacenan como aristas. Apuntar a una relación es un error irrecuperable. (REQ-665)
...
2. **Sin propiedades de relación.** Las relaciones (`-[r:TYPE]->`) existen únicamente como metadatos de unión en la capa semántica. (REQ-574) No llevan atributos almacenados, por lo que `WHERE r.since > 2020` o `RETURN r.weight` no tienen sentido y no son compatibles.
