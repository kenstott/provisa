...
- 關係無法被寫入——它們衍生自外部索引鍵 JOIN，而非儲存的邊。以關係為目標即為硬性錯誤。(REQ-665)
...
2. **無關係屬性。**關係（`-[r:TYPE]->`）於語意層中僅作為 JOIN 中繼資料存在。(REQ-574) 它們不帶有任何儲存屬性，因此 `WHERE r.since > 2020` 或 `RETURN r.weight` 並無意義，亦不受支援。
