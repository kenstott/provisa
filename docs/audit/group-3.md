...
| 250 | Trino catalog gen | Fixed 2026-06-18 (Kafka backend-unverified) | Phase 3b: `_process_kafka_sources` generates the Kafka catalog `.properties` (+ SASL client props) from YAML via `write_kafka_catalog_files`. Benefits from the same connector-resolution fix as 251. Open: this Trino runs `catalog.management=dynamic`, so the Kafka path likely needs `CREATE CATALOG` (like Prometheus) rather than dropping `.properties` files; full verify also needs a broker + schema registry |
