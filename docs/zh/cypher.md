...
- 关系无法被写入——它们派生自语义层中声明的联结，而非存储的边。以关系为目标是硬性错误。（REQ-665）由联结表支撑的边也不例外：其背后的关联表本身就是一张已注册的表，写入的行进入那张表，而不是进入边。（REQ-1586）
...
2. **关系属性只存在于由联结表支撑的边上。** 在一对外键列上声明的边，在语义层中仅作为联结元数据存在（REQ-574），不携带任何存储属性，因此 `WHERE r.since > 2020` 或 `RETURN r.weight` 对它没有意义。而在联结表上声明的边确实携带属性：关联表其余的列就是该关系的属性，`RETURN r` 会返回它们，对其中之一的 `WHERE` 会编译成作用于联结表别名的谓词——因此它约束的是遍历本身，而不是过滤已组装好的行。（REQ-1586）联结表本身会从图模式的节点一侧移除；它在这里是一条边，在其他任何地方都是一张表。
