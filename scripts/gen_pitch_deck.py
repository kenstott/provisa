...
# ── Palette ───────────────────────────────────────────────────────────────────
NAVY = RGBColor(0x1F, 0x29, 0x33)  # graphite ink — primary bg
TEAL = RGBColor(0x10, 0xB9, 0x81)  # emerald accent
TEAL_DARK = RGBColor(0x0B, 0x81, 0x5A)
...
def capability_card(slide, left, top, width, height, title, bullets):
    ...
    # Card bg
    card = add_rect(
        slide, left, top, width, height, fill_color=OFFWHITE, line_color=TEAL, line_width=1.5
    )
    # Title strip
    ...
    txb = slide.shapes.add_textbox(
        Inches(left + 0.12), Inches(top + 0.45), Inches(width - 0.22), Inches(height - 0.55)
    )
    txb.word_wrap = True
    tf = txb.text_frame
    tf.word_wrap = True
...
def arch_box(
    slide, left, top, width, height, label, sublabel=None, fill=TEAL, text_color=WHITE, font_size=13
):
    add_rect(slide, left, top, width, height, fill_color=fill, line_color=TEAL_DARK, line_width=1)
    ...
    cards = [
        (
            ...
            [
                ...
                "Fault-tolerant execution spools to object storage for large analytical workloads",
...
def slide_deployment(prs):
    ...
    for opt, (lx, ty) in zip(options, positions):
        # Card bg
        add_rect(slide, lx, ty, card_w, card_h, fill_color=WHITE, line_color=TEAL, line_width=1.5)
        # Header strip
        ...
        # Bullet list
        txb = slide.shapes.add_textbox(
            Inches(lx + 0.15), Inches(ty + 0.54), Inches(card_w - 0.25), Inches(card_h - 0.62)
        )
        txb.word_wrap = True
        tf = txb.text_frame
        tf.word_wrap = True
...
def slide_arch_overview(prs):
    ...
    slide_header(
        ...
        "Four-layer pipeline — each layer has a single responsibility",
    )
    ...
    for i, (title, sub, clr) in enumerate(layers):
        ...
        if i < len(layers) - 1:
            arrow_y = ty + box_h
            add_rect(slide, lx + box_w / 2 - 0.025, arrow_y, 0.05, gap - 0.02, fill_color=TEAL)
    ...
    for i, (ep_title, ep_sub) in enumerate(entry_points):
        ...
        add_rect(slide, ep_x, ty, ep_w, ep_h, fill_color=WHITE, line_color=TEAL, line_width=1.2)
        ...
        add_textbox(
            slide, ep_x + 0.1, ty + 0.33, ep_w - 0.15, 0.72, ep_sub, font_size=10, color=DARK_TEXT
        )
...
def slide_query_paths(prs):
    ...
    add_rect(slide, 0, 0, 13.333, 7.5, fill_color=OFFWHITE)
    slide_header(
        slide, "Query Execution Paths", "Structural routing — deterministic, not configurable"
    )
...
def slide_security_model(prs):
    ...
    slide_header(
        slide,
        "Security Model",
        "Three independent enforcement layers — no single layer depends on the others",
    )
    ...
    for i, (layer, clr) in enumerate(zip(layers, colors)):
        ...
        add_rect(slide, lx, ty, card_w, card_h, fill_color=WHITE, line_color=clr, line_width=1.5)
        # Number badge
        ...
        # Description
        add_textbox(
            slide, lx + 0.65, ty + 0.45, 11.8, 0.72, layer["desc"], font_size=11, color=DARK_TEXT
        )
        # Threat tag
        add_textbox(
            slide,
...
def slide_devops(prs):
    ...
    add_textbox(
        slide, 0.3, 1.32, 5.8, 0.3, "DEPLOYMENT LIFECYCLE", font_size=11, bold=True, color=WHITE
    )

    lifecycle = [
        (
            "Build",
            "CI produces AppImage artifact; pushed to cloud object storage (S3 / GCS / Azure Blob)",
        ),
        ("Provision", "Terraform apply — VMs provisioned, LB configured, firewall rules set"),
        (
            "Bootstrap",
            "Startup script: pull AppImage from storage, chmod +x, launch with --role flag",
        ),
        (
            "Scale Out",
            "Add worker nodes: Terraform node_count + 1; workers self-register with coordinator",
        ),
        (
            "Upgrade",
            "Rolling: terminate workers one at a time, pull new AppImage, restart; coordinator last",
        ),
    ...
    for i, (step, desc) in enumerate(lifecycle):
        ...
        add_textbox(slide, 1.38, ty + 0.06, 4.7, 0.54, desc, font_size=10.5, color=DARK_TEXT)

    # ── Right: Monitoring Stack ───────────────────────────────────────────────
    add_rect(slide, 6.7, 1.28, 6.4, 0.38, fill_color=NAVY)
    add_textbox(
        slide, 6.82, 1.32, 6.2, 0.3, "OBSERVABILITY STACK", font_size=11, bold=True, color=WHITE
    )
    ...
    for title, points in monitoring:
        ...
        add_textbox(slide, 6.82, my + 0.03, 6.2, 0.27, title, font_size=11, bold=True, color=WHITE)
        my += 0.34

        txb = slide.shapes.add_textbox(
            Inches(6.82), Inches(my), Inches(6.2), Inches(len(points) * 0.3 + 0.05)
        )
        txb.word_wrap = True
        tf = txb.text_frame
        tf.word_wrap = True
...
def slide_closing(prs):
    ...
    for title, sub in pillars:
        add_textbox(slide, px, 3.3, 3.0, 0.4, title, font_size=18, bold=True, color=TEAL)
        add_textbox(slide, px, 3.72, 3.0, 0.45, sub, font_size=11, color=LIGHT_GRAY)
        px += 3.15
