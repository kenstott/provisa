...
The whole model comes down to two contracts and two policies: sources reduce to 2-D tables over one type system, queries reduce to one SQL-like IR, reachability decides what is queried live versus materialized, and a freshness strategy governs each materialized copy. Data shape in, query shape in, governance at the join, native queries out. The rest of this section walks each piece.
