...
Enable by setting `PROVISA_PGWIRE_PORT` (e.g. `5433`) before starting Provisa. Disabled when unset or `0`. (REQ-527)

### Why pgwire instead of JDBC?

The JDBC driver uses Arrow Flight as its transport and requires deploying the `provisa-jdbc.jar`. (REQ-293) pgwire requires nothing — if you already have `psql`, DBeaver, SQLAlchemy, or a PG JDBC driver, you are done. It is the lower-friction path for SQL-only workloads.

JDBC is the right choice for BI tools that have a built-in JDBC connection wizard and benefit from Arrow Flight's columnar streaming for large result sets. (REQ-293, REQ-045) pgwire accepts free SQL against the full published schema — the same queries, lower setup cost.
...
pgwire uses cleartext password auth bridged to Provisa's configured auth provider (`none` or `simple`). (REQ-529) In trust mode (`none`), the username maps directly to a role — password is ignored. (REQ-529) MD5 is not supported; enable TLS (`PROVISA_PGWIRE_CERT` / `PROVISA_PGWIRE_KEY`) when running over an untrusted network. (REQ-530)

### Limitations

- SQL only. GraphQL and Cypher are not accepted over pgwire. (REQ-614)
- Read-only. Mutations are not supported. (REQ-615)
- `COPY`, DDL (`CREATE TABLE`, etc.) require the `ddl` role capability; operations without it are rejected with a permission error. (REQ-616)
- `information_schema` and `pg_catalog` queries are intercepted and answered from a DuckDB catalog shim — schema discovery tools work correctly. (REQ-532)

---

## JDBC Driver

The Provisa JDBC driver uses Arrow Flight as its underlying transport. (REQ-293) It is the recommended path for BI tools with a JDBC connection wizard.
...
Authentication uses standard JDBC `user` / `password` properties. (REQ-273) Provisa authenticates the credentials against the configured auth provider and assigns the role — the client does not choose its own role. (REQ-273)
...
Arrow Flight (port 8815) is the recommended path for data tools that support it. (REQ-143) Results stream as Arrow RecordBatches without materializing in Provisa memory. (REQ-145)
...
Service-to-service path. (REQ-525) Provisa generates a `.proto` per role at startup — each role sees only the tables and columns it has access to. (REQ-525)
...
Use `grpc_server_reflection` to discover the schema programmatically. (REQ-526)

Role is passed via the `x-provisa-role` metadata key on every RPC. (REQ-617) Streaming queries emit one message per row; mutations are unary. (REQ-617)

---

## Apollo Federation

Provisa can act as a Federation v2 subgraph, exposing its published schema to an Apollo Router or Apollo Gateway. (REQ-259)
...
Provisa generates `@key` directives on primary-key columns and `@external`/`@provides` on cross-subgraph relationships automatically. (REQ-259)
...
Provisa responds to `_entities` queries for cross-subgraph joins. (REQ-259) Any table with a primary key is automatically resolvable as a Federation entity. (REQ-259)
...
See [docs/sources.md](sources.md#kafka) for Kafka topic configuration as read-only tables and query result sinks. (REQ-147, REQ-176)
