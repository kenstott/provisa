...
become underscores) with connection properties: (REQ-183)
...
|---|---|---|
| **DataConnectorLink** | `sources[]` | Source type inferred from connector URL (postgres, mysql, mssql, mongo, clickhouse, snowflake, bigquery). Connection details default to placeholders; use `--source-overrides` to set actual values. (REQ-183) |
| **ObjectType** | Column definitions on `tables[]` | Fields become columns. `dataConnectorTypeMapping.fieldMapping` resolves GraphQL field names to physical column names. (REQ-189) |
| **Model** | `tables[]` | Each Model produces one table. `source_id` from connector, `table_name` from collection. `graphql_type_name` becomes `alias`. (REQ-183) |
| **Relationship** | `relationships[]` | Object type -> `many-to-one`, Array type -> `one-to-many`. Field mapping resolved through physical column lookup. (REQ-183, REQ-189) |
| **TypePermissions** | `columns[].visible_to[]` | `allowedFields` determines which roles can see each column. (REQ-185) |
| **ModelPermissions** | `rls_rules[]` | Filter predicates converted to SQL WHERE clauses. Supports `_eq`, `_neq`, `_gt`, `_lt`, `_gte`, `_lte`, `_in`, `_nin`, `_like`, `_is_null`, `_and`, `_or`, `_not`. Session variable references preserved as `${x-hasura-...}`. (REQ-184, REQ-187) |
| **Command** | `functions[]` | Both functions and procedures mapped. Arguments, return type, and GraphQL root field name preserved. `domain_id` set from subgraph. (REQ-192) |
| **AggregateExpression** | Sidecar YAML output | Count, count_distinct, and per-field aggregate functions written to a sidecar `<output-stem>-aggregates.yaml` file keyed by object type name. (REQ-191) |
...
(column names) via `dataConnectorTypeMapping` on ObjectTypes. The converter: (REQ-189)
...
and sets `columns[].alias` to the GraphQL field name when they differ. (REQ-189)
...
4. **Check RLS rules.** DDN filter predicates are converted to SQL approximations. (REQ-187)
   Nested boolean logic (`_and`/`_or`/`_not`) is supported but complex
   relationship-traversing filters may need manual review.
5. **Review aggregate annotations.** Aggregate expressions are written to a sidecar
   `<output-stem>-aggregates.yaml` file keyed by object type name. (REQ-191) If you need this
   data in the main config, parse the sidecar or configure aggregates separately.
6. **Review warnings.** The converter prints a summary to stderr listing skipped
   DDN Kinds and any models referencing unknown ObjectTypes. (REQ-192)
...
defaults to `postgresql`. Override with `--source-overrides`. (REQ-183)
...
in the scanned directory. REQ-628

### Subgraph discovery

Subgraphs are discovered from the directory structure of HML files. The first
directory component under the project root is used as the subgraph name (e.g.
`app/subgraph1/models/MyModel.hml` → subgraph `subgraph1`). Files under
`globals/` are assigned to the `globals` subgraph and excluded from domain
discovery. REQ-629

### Relationship source resolution

Relationships reference a `source_type` (ObjectType name) and `target_model` (Model
name). If no Model uses the given ObjectType, the relationship is skipped silently. (REQ-183)
...
`alias` is the GraphQL name your application used. (REQ-189)

### Aggregate expressions

Aggregate expressions are not a first-class Provisa config section. They are written
to a sidecar `<output-stem>-aggregates.yaml` file (keyed by object type name) rather
than embedded in the main `provisa.yaml`. (REQ-191) If you need programmatic access,
parse the sidecar or configure aggregates separately.
