...
async def test_an_entitled_org_gets_both_sync_jobs_on_its_own_schedule(org):
    ...
    await sync.register_org_jobs(scheduler, ORG_ID)

    drain = scheduler.get_job(sync.drain_job_id(ORG_ID))
    reconcile = scheduler.get_job(sync.reconcile_job_id(ORG_ID))
    ...
    assert drain.func is sync.drain_org
    assert reconcile.func is sync.reconcile_org
...
async def test_an_unentitled_org_has_its_sync_jobs_disarmed(org):
    ...
    assert scheduler.get_job(sync.drain_job_id(ORG_ID)) is None
    assert scheduler.get_job(sync.reconcile_job_id(ORG_ID)) is None
...
async def test_a_scheduled_reconcile_for_an_unentitled_org_is_skipped_not_raised(org):
    ...
    org("standard")
    assert await sync.reconcile_org(ORG_ID) is None
...
async def test_an_unentitled_org_queues_no_work_when_the_model_changes(org):
    ...
    org("standard")
    assert await sync.notify_model_changed(ORG_ID, reason="schema rebuild") is None
