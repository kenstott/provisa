...
async def test_cadence_not_elapsed_skips_no_probe():
    probed = []
    e = ScheduledEntry(
        "a", _policy(cadence=3600, probe_capable=True), last_refresh_at=100.0, stored_token="t"
    )
...
async def test_probe_unchanged_resets_clock_no_land():
    e = ScheduledEntry(
        "a", _policy(cadence=60, probe_capable=True), last_refresh_at=100.0, stored_token="tok"
    )
...
async def test_probe_changed_lands():
    e = ScheduledEntry(
        "a", _policy(cadence=60, probe_capable=True), last_refresh_at=100.0, stored_token="old"
    )
...
async def test_off_window_skips_before_probe():
    ...
    e = ScheduledEntry(
        "a", _policy(window=w, probe_capable=True), last_refresh_at=None, stored_token=None
    )
