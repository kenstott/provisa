...
"""MetadataEgress port, registry and config tests (REQ-1068)."""
...
from provisa.api.metadata_egress import registry as registry_module
from provisa.api.metadata_egress.model import AssetKind, AssetRef, MetadataSnapshot
...
def _stub(name: str) -> type[MetadataEgress]:
    class _Stub(MetadataEgress):
        provider_name = name
...
def _enabled(**overrides) -> MetadataEgressConfig:
    base = {"enabled": True, "provider": "stub", "endpoint": "https://catalog.example.com"}
    base.update(overrides)
    return MetadataEgressConfig(**base)
...
def test_factory_resolves_each_registered_name(isolated_registry):
    ...
    for name in ("openlineage", "openmetadata", "atlas"):
        provider = isolated_registry.metadata_egress(_enabled(provider=name))
...
def test_unknown_provider_is_refused_at_construction(isolated_registry):
    ...
    with pytest.raises(MetadataEgressNotConfiguredError) as exc:
        isolated_registry.metadata_egress(_enabled(provider="nope"))
...
def test_disabled_config_refuses_rather_than_no_opping(isolated_registry):
    isolated_registry.register_provider(_stub("openlineage"))
    with pytest.raises(MetadataEgressNotConfiguredError) as exc:
        isolated_registry.metadata_egress(
            MetadataEgressConfig(enabled=False, provider="openlineage")
        )
    assert "metadata_egress.enabled" in str(exc.value)


def test_provider_without_a_name_fails_registration(isolated_registry):
    class Nameless(MetadataEgress):
...
def test_module_level_factory_is_the_same_function():
    ...
    # callers reach.
    assert metadata_egress is registry_module.metadata_egress
...
def test_enabled_without_provider_or_endpoint_is_rejected():
    with pytest.raises(ValidationError, match="provider, endpoint"):
        MetadataEgressConfig(enabled=True)
    with pytest.raises(ValidationError, match="endpoint"):
        MetadataEgressConfig(enabled=True, provider="atlas")
    with pytest.raises(ValidationError, match="provider"):
        MetadataEgressConfig(enabled=True, endpoint="https://catalog.example.com")


def test_disabled_config_needs_nothing_else():
    assert MetadataEgressConfig().enabled is False
...
def test_credentials_are_redacted_in_repr_and_logs(caplog):
    ...
    with caplog.at_level(logging.INFO):
        logging.getLogger(__name__).info("egress config: %s", config)
...
def test_config_is_mounted_on_provisa_config():
    ...
    assert "metadata_egress" in ProvisaConfig.model_fields
