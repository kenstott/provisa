...
"""REQ-1165: the scheduled-refresh preflight gate — per-input SQL pushdown + Arrow streaming.

The gate runs before materialization and its subject is the MV's INPUTS, streamed per input node:
a SQL-expressible check is pushed down as a count probe over the named input (no rows to Python),
a non-SQL check opens one lazy Arrow stream per input and short-circuits. A non-SQL check REQUIRES
ARROW_STREAM — on an engine without it the gate fails loud (no materialize fallback). Anything but
CONTINUE skips the rebuild (ABORT → STALE + error; QUARANTINE → SKIPPED_PREFLIGHT hold).
"""
...
class _Engine:
    ...
    the source-size probe (contains ``_probe``) returns ``size``. ``execute_engine_stream`` backs a
    per-input Arrow stream from ``arrow_rows``; ``caps`` gates the ARROW_STREAM requirement."""
    ...
    def require(self, cap) -> None:
        if cap not in self._caps:
            raise UnsupportedCapabilityError(self.name, cap)

    def execute_engine_stream(self, sql, *a, **k):
        self.sqls.append(sql)  # per-input scan, e.g. SELECT * FROM "orders"
        return object(), iter(pa.Table.from_pylist(self.arrow_rows).to_batches())
...
async def test_sql_pushdown_abort_and_continue() -> None:
    ...
    # the probe was pushed down over the INPUT NODE (no per-row Python) — one count(*) over orders
    assert any("_preflight" in s and "count(*)" in s and "orders" in s for s in eng.sqls)
...
async def test_non_sql_streams_arrow_and_quarantines() -> None:
    eng = _Engine(
        arrow_rows=[{"id": 1, "qty": 5}, {"id": 2, "qty": 6}, {"id": 3, "qty": 7}],
        caps=frozenset({EngineCapability.ARROW_STREAM}),
    )
    ...
    # never pushed down (no _preflight probe) — evaluated over the input's Arrow batches instead
    assert not any("_preflight" in s for s in eng.sqls)


@pytest.mark.asyncio
async def test_non_sql_without_arrow_stream_fails_loud() -> None:
    # REQ-1165: a streaming check on an engine lacking ARROW_STREAM fails loud — no materialize fallback.
    eng = _Engine(arrow_rows=[{"id": 1, "qty": 1}])  # caps empty → no ARROW_STREAM
    with pytest.raises(UnsupportedCapabilityError):
        await _evaluate_preflight(eng, _mv(preprocess=_NON_SQL), "SELECT id, qty FROM orders")
...
async def test_refresh_mv_quarantine_holds() -> None:
    eng = _Engine(
        size=1,
        arrow_rows=[{"id": i, "qty": i} for i in range(4)],
        caps=frozenset({EngineCapability.ARROW_STREAM}),
    )
...
@pytest.mark.asyncio
async def test_arrow_stream_used_for_non_sql_check() -> None:
    # When ARROW_STREAM is advertised, the lazy reader is driven per input (schema, batch-generator).
