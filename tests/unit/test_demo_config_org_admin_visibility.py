...
"""REQ-1323: the shipped demo config must be visible to ``org_admin``.

``config/provisa.yaml`` is the config every self-created demo org loads (``include_demo``), and
the org's creator holds exactly one role there: ``org_admin``. Column visibility is strict
membership — ``schema_gen._build_visible_tables`` keeps a column only when its ``visible_to`` is
empty or names the role, and drops the whole table when nothing survives. So a demo column listing
...
def test_every_restricted_demo_function_and_webhook_is_visible_to_org_admin(demo_config):
    callables = demo_config.get("functions", []) + demo_config.get("webhooks", [])
    hidden = [
