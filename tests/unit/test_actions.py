...
"""Unit tests for the Actions feature — tracked DB functions and webhooks
exposed as GraphQL mutations (REQ-205 through REQ-211).

Covers pure logic with no real DB or HTTP connections:
- Function SQL generation
- Webhook mutation field generation
- Webhook HTTP dispatch (mocked httpx)
...
class TestFunctionSQLGeneration:
    def test_build_function_sql_no_args(self):
        ...
        assert 'public"."do_action"()' in sql or "do_action()" in sql
        assert params == []

    def test_build_function_sql_with_args(self):
        """Function with args produces parameterised placeholders."""
        func = Function(
            name="doAction",
            source_id="pg",
            schema="public",
        ...
        assert "us-east" in params or "$1" in sql
        assert 5 in params or "$2" in sql
        assert len(params) == 2

    def test_build_function_sql_special_schema(self):
        """Non-public schema is correctly quoted in generated SQL."""
        func = Function(
            name="myFunc",
            source_id="pg",
            schema="analytics",
            function_name="compute_segment",
            returns="pg.analytics.customer_segments",
        )
        sql, _ = build_function_sql(func, [])
        assert "analytics" in sql
        assert "compute_segment" in sql
