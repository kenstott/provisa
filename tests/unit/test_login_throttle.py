...
def _pgwire_attempt(auth_config: dict, password: str) -> list[tuple]:
    ...
    # A real handler holds the CountingWriter that meters egress as ``_meter`` (and installs it as
    # ``wfile``); a successful auth binds the org onto it (REQ-1452), so the double has to carry it.
