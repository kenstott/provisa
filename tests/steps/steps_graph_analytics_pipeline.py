...
# ---------------------------------------------------------------------------
# Core pipeline logic (mirrors the /data/graph-analytics endpoint contract).
# ---------------------------------------------------------------------------
...
def _execute_graph_analytics(request: dict) -> dict:
    ...
    # The Cypher query result — in a real router this comes from cypher_router.
    nodes = request["nodes"]
    edges = request["edges"]
...
def given_graph_analytics_request(shared_data: dict):
    # A small Cypher-defined subgraph: result nodes and edges the router would
    # produce from running the supplied Cypher query.
    nodes = [
...
def then_returns_augmented(shared_data: dict):
    ...
    # PageRank scores form a probability distribution summing to ~1.0.
    total = sum(n["_analytics"]["score"] for n in response["nodes"])
    assert abs(total - 1.0) < 1e-6

    # Every edge has a merged _analytics dict.
...
# Copyright (c) 2026 Kenneth Stott
# Canary: 29c9ea53-e447-4807-82a8-9305034c9a67
#
# This source code is licensed under the Business Source License 1.1
# found in the LICENSE file in the root directory of this source tree.
"""pytest-bdd step implementations for REQ-642 - Graph Analytics Pipeline."""
...
# Copyright (c) 2026 Kenneth Stott
# Canary: 3a0280e3-9b7b-4d65-bade-de5248a9c5e9
#
# This source code is licensed under the Business Source License 1.1
