...
def select_masked_where_join_physical(shared_data: dict) -> None:
    ...
    # SELECT projects the masked expression for the joined column.
    assert "REGEXP_REPLACE" in select_part_join.upper(), (
        f"Expected REGEXP_REPLACE in SELECT portion; got: {select_part_join!r}"
    )
    # The JOIN ON condition uses physical unmasked columns.
    assert 'ON "orders"."customer_id" = "customers"."id"' in rest_part_join, (
        f"JOIN ON must reference physical columns; got: {rest_part_join!r}"
    )
    # No masking leaked into the JOIN ON / WHERE region.
    assert "REGEXP_REPLACE" not in rest_part_join.upper(), (
        f"REGEXP_REPLACE leaked past SELECT: {rest_part_join!r}"
    )
...
def masked_column_across_source_types(shared_data: dict) -> None:
    # The same masking rules are exercised against columns whose physical source
    # is postgres, mysql, iceberg, etc. build_mask_expression must not vary its
    # output by source dialect — it always emits ANSI/Trino SQL.
    shared_data["source_types"] = ["postgresql", "mysql", "iceberg", "sqlserver", "oracle"]

    shared_data["regex_rule"] = MaskingRule(
        mask_type=MaskType.regex, pattern=r".+@", replace="***@"
    )
    shared_data["truncate_rule"] = MaskingRule(
        mask_type=MaskType.truncate, precision="month"
    )
    shared_data["column_ref"] = '"t0"."email"'
    shared_data["ts_column_ref"] = '"t0"."created_at"'


@when("build_mask_expression generates the mask")
def build_mask_across_source_types(shared_data: dict) -> None:
    regex_outputs = {}
    truncate_outputs = {}
    for _src in shared_data["source_types"]:
        # build_mask_expression takes no source-type argument: the same rule
        # produces the same ANSI SQL for every physical source.
        regex_outputs[_src] = build_mask_expression(
            shared_data["regex_rule"], shared_data["column_ref"], "varchar"
        )
        truncate_outputs[_src] = build_mask_expression(
            shared_data["truncate_rule"], shared_data["ts_column_ref"], "timestamp"
        )
    shared_data["regex_outputs"] = regex_outputs
    shared_data["truncate_outputs"] = truncate_outputs


@then("output is ANSI REGEXP_REPLACE/DATE_TRUNC regardless of source dialect")
def output_is_ansi_regardless_of_source(shared_data: dict) -> None:
    regex_outputs = shared_data["regex_outputs"]
    truncate_outputs = shared_data["truncate_outputs"]

    # Every source produced identical, ANSI/Trino SQL — no dialect divergence.
    distinct_regex = set(regex_outputs.values())
    distinct_truncate = set(truncate_outputs.values())
    assert len(distinct_regex) == 1, f"regex output varied by source: {regex_outputs}"
    assert len(distinct_truncate) == 1, f"truncate output varied by source: {truncate_outputs}"

    regex_sql = distinct_regex.pop()
    truncate_sql = distinct_truncate.pop()

    assert regex_sql == "REGEXP_REPLACE(\"t0\".\"email\", '.+@', '***@')", regex_sql
    assert truncate_sql == "DATE_TRUNC('month', \"t0\".\"created_at\")", truncate_sql
...
def masking_rule_incompatible_type(shared_data: dict) -> None:
    # Three incompatible (rule, column) combinations the requirement enumerates.
    shared_data["incompatible"] = [
        # regex on an integer column
        (
            MaskingRule(mask_type=MaskType.regex, pattern=r"\d", replace="#"),
            "user_id",
            "integer",
            True,
        ),
        # truncate on a varchar column
        (
        ...
        # NULL constant on a NOT NULL column
        (
            MaskingRule(mask_type=MaskType.constant, value=None),
            "email",
            "varchar",
            False,  # is_nullable=False
        ),
    ]
    # A compatible control that must NOT be rejected.
    shared_data["compatible"] = (
        MaskingRule(mask_type=MaskType.regex, pattern=r".+@", replace="***@"),
        "email",
        "varchar",
        True,
    )
...
def config_is_loaded(shared_data: dict) -> None:
    results = []
    for rule, col, dtype, nullable in shared_data["incompatible"]:
        try:
            validate_masking_rule(rule, col, dtype, nullable)
            results.append(None)  # no error → validation failed to reject
        except MaskingValidationError as exc:
            results.append(str(exc))
    shared_data["validation_results"] = results

    # The compatible rule must load cleanly.
    rule, col, dtype, nullable = shared_data["compatible"]
    shared_data["compatible_error"] = None
    try:
        validate_masking_rule(rule, col, dtype, nullable)
    except MaskingValidationError as exc:  # pragma: no cover - control must pass
        shared_data["compatible_error"] = str(exc)


@then(
    "validation rejects the rule (e.g., regex on integer, truncate on varchar, NULL on NOT NULL)"
)
def validation_rejects_incompatible(shared_data: dict) -> None:
    results = shared_data["validation_results"]

    # Every incompatible combination was rejected (non-None error message).
    assert all(r is not None for r in results), f"some rules were not rejected: {results}"
    assert len(results) == 3

    regex_err, truncate_err, null_err = results
    assert "regex masking is only supported on string types" in regex_err, regex_err
    assert "truncate masking is only supported on date/timestamp types" in truncate_err, (
        truncate_err
    )
    assert "cannot use NULL as constant mask value" in null_err, null_err

    # The compatible control was NOT rejected.
    assert shared_data["compatible_error"] is None, shared_data["compatible_error"]
...
def various_constant_mask_values(shared_data: dict) -> None:
    # (rule, data_type, expected_sql_literal)
    shared_data["constant_cases"] = [
        (MaskingRule(mask_type=MaskType.constant, value=None), "varchar", "NULL"),
        (MaskingRule(mask_type=MaskType.constant, value=True), "boolean", "TRUE"),
        (MaskingRule(mask_type=MaskType.constant, value=False), "boolean", "FALSE"),
        ...
        (
            MaskingRule(mask_type=MaskType.constant, value="O'Brien"),
            "varchar",
            "'O''Brien'",
        ),
        (
            MaskingRule(mask_type=MaskType.constant, value="redacted"),
            "varchar",
            "'redacted'",
        ),
    ]
...
def build_constant_literals(shared_data: dict) -> None:
    outputs = []
    for rule, dtype, _expected in shared_data["constant_cases"]:
        outputs.append(build_mask_expression(rule, '"t0"."col"', dtype))
    shared_data["constant_outputs"] = outputs


@then(
    "output is syntactically valid (NULL keyword, TRUE/FALSE, numeric unquoted, "
    "strings single-quoted with escaped apostrophes)"
)
def constant_literals_valid(shared_data: dict) -> None:
    outputs = shared_data["constant_outputs"]
    cases = shared_data["constant_cases"]

    for (rule, dtype, expected), actual in zip(cases, outputs):
        assert actual == expected, f"value={rule.value!r} type={dtype}: {actual!r} != {expected!r}"

    # NULL is the bare keyword, not a quoted string.
    assert outputs[0] == "NULL"
    # Booleans are bare keywords.
    assert outputs[1] == "TRUE" and outputs[2] == "FALSE"
    # Numerics are unquoted.
    assert outputs[3] == "42" and "'" not in outputs[3]
    assert outputs[4] == "3.14" and "'" not in outputs[4]
    # String with apostrophe is single-quoted with the apostrophe doubled (escaped).
    assert outputs[5] == "'O''Brien'"
    # Every literal parses as valid SQL.
    import sqlglot

    for out in outputs:
        sqlglot.parse_one(f"SELECT {out}", read="trino")
...
def query_with_structural_clauses(shared_data: dict) -> None:
    # The masked column 'email' appears in the SELECT projection only; the
    # surrounding GROUP BY / ORDER BY / LIMIT structure must survive untouched.
    role_id = "analyst"
    shared_data["role_id"] = role_id
    ...
    shared_data["masking_rules"] = {
        (_CUSTOMERS_TABLE_ID, role_id): {"email": (mask_rule, "varchar")}
    }
    ...
    shared_data["sql_744"] = sql_744
    shared_data["compiled_744"] = CompiledQuery(
        sql=sql_744,
        params=[],
        root_field="customers",
        columns=[
...
def clauses_unchanged_new_object(shared_data: dict) -> None:
    result = shared_data["result_744"]
    original = shared_data["compiled_744"]

    # Masking returned a distinct object; the input CompiledQuery is unmutated.
    assert result is not original, "inject_masking must return a new CompiledQuery"
    assert original.sql == shared_data["sql_744"], "input SQL was mutated"

    from provisa.compiler.mask_inject import _find_select_end

    sql = result.sql
    rest = sql[_find_select_end(sql):]

    # The masked expression is in the SELECT projection.
    select_part = sql[: _find_select_end(sql)]
    assert "REGEXP_REPLACE" in select_part.upper(), select_part

    # Structural clauses survive verbatim in the non-SELECT remainder.
    assert 'WHERE "active" = TRUE' in rest, rest
    assert 'GROUP BY "email"' in rest, rest
    assert 'ORDER BY "n" DESC' in rest, rest
    assert "LIMIT 100" in rest, rest
    # No masking leaked outside the SELECT projection.
    assert "REGEXP_REPLACE" not in rest.upper(), rest
...
def same_column_different_rules_per_role(shared_data: dict) -> None:
    # Column 'email' on the customers table with three distinct role treatments:
    #   admin        → no rule (raw values)
    #   analyst      → regex mask
    #   masked_viewer → constant mask
    regex_rule = MaskingRule(
        mask_type=MaskType.regex, pattern=r"^(.{2}).*(@.*)$", replace=r"\1***\2"
    )
    constant_rule = MaskingRule(mask_type=MaskType.constant, value="REDACTED")

    shared_data["masking_rules_745"] = {
        (_CUSTOMERS_TABLE_ID, "analyst"): {"email": (regex_rule, "varchar")},
        (_CUSTOMERS_TABLE_ID, "masked_viewer"): {"email": (constant_rule, "varchar")},
        # admin intentionally absent → no masking.
    }

    ctx = CompilationContext()
    ctx.tables = {"customers": _customers_meta_740()}
    ctx.joins = {}
    shared_data["ctx_745"] = ctx

    shared_data["base_sql_745"] = 'SELECT "email", "name" FROM "public"."customers"'
    shared_data["compiled_745"] = CompiledQuery(
        sql=shared_data["base_sql_745"],
        params=[],
        root_field="customers",
        columns=[
            ColumnRef(alias=None, column="email", field_name="email", nested_in=None),
...
def inject_masking_for_each_role(shared_data: dict) -> None:
    rules = shared_data["masking_rules_745"]
    ctx = shared_data["ctx_745"]
    compiled = shared_data["compiled_745"]

    shared_data["result_admin"] = inject_masking(compiled, ctx, rules, "admin")
    shared_data["result_analyst"] = inject_masking(compiled, ctx, rules, "analyst")
    shared_data["result_masked_viewer"] = inject_masking(compiled, ctx, rules, "masked_viewer")


@then("admin sees raw values; analyst sees regex mask; masked_viewer sees constant mask")
def roles_see_different_masks(shared_data: dict) -> None:
    admin_sql = shared_data["result_admin"].sql
    analyst_sql = shared_data["result_analyst"].sql
    viewer_sql = shared_data["result_masked_viewer"].sql

    # admin: no rule → SQL unchanged, raw column projected, no mask expression.
    assert admin_sql == shared_data["base_sql_745"], admin_sql
    assert "REGEXP_REPLACE" not in admin_sql.upper()
    assert "REDACTED" not in admin_sql

    # analyst: regex mask applied to email.
    assert "REGEXP_REPLACE" in analyst_sql.upper(), analyst_sql
    assert "'^(.{2}).*(@.*)$'" in analyst_sql, analyst_sql
    assert "REDACTED" not in analyst_sql

    # masked_viewer: constant mask applied to email.
    assert "'REDACTED'" in viewer_sql, viewer_sql
    assert "REGEXP_REPLACE" not in viewer_sql.upper()

    # All three differ from each other — role-specific masking is real.
    assert admin_sql != analyst_sql != viewer_sql != admin_sql
...
def role_with_specific_capability(shared_data: dict) -> None:
    shared_data["granted"] = Capability.QUERY_DEVELOPMENT
    shared_data["missing"] = Capability.SOURCE_REGISTRATION

    # Role carries QUERY_DEVELOPMENT but NOT SOURCE_REGISTRATION, and is not admin.
    shared_data["role"] = {
        "id": "developer",
        "capabilities": [Capability.QUERY_DEVELOPMENT.value],
    }
    assert has_capability(shared_data["role"], Capability.QUERY_DEVELOPMENT)
    assert not has_capability(shared_data["role"], Capability.SOURCE_REGISTRATION)
...
def call_check_capability(shared_data: dict) -> None:
    ...
    # Granted capability: no exception.
    shared_data["granted_error"] = None
    try:
        check_capability(role, shared_data["granted"])
    except InsufficientRightsError as exc:  # pragma: no cover - must not raise
        shared_data["granted_error"] = exc

    # Missing capability: must raise.
    shared_data["missing_error"] = None
    try:
        check_capability(role, shared_data["missing"])
    except InsufficientRightsError as exc:
        shared_data["missing_error"] = exc


@then(
    "no exception is raised; for missing capability, InsufficientRightsError is raised"
)
def capability_enforcement_verified(shared_data: dict) -> None:
    # Granted capability passed without raising.
    assert shared_data["granted_error"] is None, shared_data["granted_error"]

    # Missing capability raised InsufficientRightsError identifying role + capability.
    err = shared_data["missing_error"]
    assert isinstance(err, InsufficientRightsError), err
    assert err.role_id == "developer"
    assert err.required == shared_data["missing"]

    # ADMIN capability is a universal grant — a distinct admin role passes any check.
    admin_role = {"id": "root", "capabilities": [Capability.ADMIN.value]}
    check_capability(admin_role, shared_data["missing"])  # must not raise
    assert has_capability(admin_role, shared_data["missing"])
...
def _remote_meta_747(table_id: int, table_name: str, source_id: str) -> TableMeta:
    return TableMeta(
        table_id=table_id,
...
def _gov_ctx_747(metas: list[TableMeta]) -> GovernanceContext:
    return GovernanceContext(
        rls_rules={},
        masking_rules={},
        visible_columns={},
        table_map={f"graphql_remote.{m.table_name}": m.table_id for m in metas}
        | {m.table_name: m.table_id for m in metas},
        all_columns={},
        limit_ceiling=None,
        sample_size=None,
    )
...
@given(
    "two remote tables from the same source_id with bypass_uncovered_relationships=True"
)
def two_remote_tables_same_source(shared_data: dict) -> None:
    # Same-source pair: bypass should suppress V002.
    animals = _remote_meta_747(101, "animals", "shelter_remote")
    breeds = _remote_meta_747(102, "breeds", "shelter_remote")
    ctx = CompilationContext()
    ctx.tables = {"animals": animals, "breeds": breeds}
    ctx.joins = {}  # NO registered relationship → uncovered pair.
    shared_data["ctx_same"] = ctx
    shared_data["gov_ctx_same"] = _gov_ctx_747([animals, breeds])

    # Cross-source pair (different source_id): coverage still required → V002 stays.
    animals_x = _remote_meta_747(201, "animals", "shelter_remote")
    breeds_x = _remote_meta_747(202, "breeds", "other_remote")
    ctx_x = CompilationContext()
    ctx_x.tables = {"animals": animals_x, "breeds": breeds_x}
    ctx_x.joins = {}
    shared_data["ctx_cross"] = ctx_x
    shared_data["gov_ctx_cross"] = _gov_ctx_747([animals_x, breeds_x])

    shared_data["role_747"] = {"id": "analyst", "domain_access": ["shelter"]}

    # Join on columns with no registered relationship — normally a V002.
    shared_data["join_sql"] = (
        'SELECT "animals"."id" '
        'FROM "graphql_remote"."animals" AS "animals" '
        'JOIN "graphql_remote"."breeds" AS "breeds" '
        'ON "animals"."breed_id" = "breeds"."id"'
    )


@when("SQL validator checks the join")
def validator_checks_remote_join(shared_data: dict) -> None:
    # Same-source pair WITH bypass enabled.
    shared_data["same_bypass"] = validate_sql(
        shared_data["join_sql"],
        shared_data["ctx_same"],
        shared_data["gov_ctx_same"],
        shared_data["role_747"],
        [],
        bypass_uncovered_relationships=True,
    )
    # Same-source pair WITHOUT bypass → V002 must fire (proves bypass is load-bearing).
    shared_data["same_no_bypass"] = validate_sql(
        shared_data["join_sql"],
        shared_data["ctx_same"],
        shared_data["gov_ctx_same"],
        shared_data["role_747"],
    ...
    # Cross-source pair WITH bypass → coverage still required, V002 must fire.
    shared_data["cross_bypass"] = validate_sql(
        shared_data["join_sql"],
        shared_data["ctx_cross"],
        shared_data["gov_ctx_cross"],
        shared_data["role_747"],
        [],
        bypass_uncovered_relationships=True,
    )
...
def v002_bypassed_for_same_source_only(shared_data: dict) -> None:
    same_bypass_codes = {v.code for v in shared_data["same_bypass"]}
    same_no_bypass_codes = {v.code for v in shared_data["same_no_bypass"]}
    cross_bypass_codes = {v.code for v in shared_data["cross_bypass"]}

    # Same-source + bypass: V002 suppressed.
    assert "V002" not in same_bypass_codes, shared_data["same_bypass"]

    # Same-source without bypass: V002 fires — the bypass flag is what suppresses it.
    assert "V002" in same_no_bypass_codes, (
        f"expected V002 without bypass, got {same_no_bypass_codes}"
    )

    # Cross-source + bypass: coverage still required, V002 remains.
    assert "V002" in cross_bypass_codes, (
        f"cross-source join must still require coverage, got {cross_bypass_codes}"
    )
