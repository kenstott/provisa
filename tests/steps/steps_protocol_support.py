...
# Canary: cbdb1759-9c05-4bba-a690-5470d1175d55
#
...
def step_server_accepts_handshake(shared_data: dict) -> None:
    ...
    assert chosen != b"\x00\x00\x00\x00", "Server rejected all version proposals; got {}".format(
        chosen.hex()
    )
    from provisa.bolt.messages import SUPPORTED_VERSIONS

    major, minor = chosen[3], chosen[2]
    assert (major, minor) in SUPPORTED_VERSIONS, "Server chose unsupported version ({}, {})".format(
        major, minor
    )
...
def step_query_transpiled_to_sql(shared_data: dict) -> None:
    """Verify that the RUN response carries a SUCCESS (not FAILURE)."""
    if not os.getenv("PROVISA_INTEGRATION"):
        pytest.skip("integration only")
    ...
    tag = run_response[1]
    assert tag != FAILURE, "Server returned FAILURE for RUN; transpilation failed. raw={}".format(
        run_response.hex()
    )
    assert tag == SUCCESS, "Expected SUCCESS(0x70) for RUN response, got 0x{:02X}".format(tag)


@pytest.mark.integration
@then("governance (RLS, masking, visibility) is applied at compile time")
def step_governance_applied(shared_data: dict) -> None:
    """Assert governance was applied: pipeline completed without FAILURE."""
    if not os.getenv("PROVISA_INTEGRATION"):
        pytest.skip("integration only")
    ...
    assert pull_final_tag != FAILURE, (
        "Governance pipeline raised FAILURE during PULL; raw={}".format(
            shared_data.get("pull_final", b"").hex()
        )
    )
...
def step_results_as_bolt_structures(shared_data: dict) -> None:
    ...
    records: list = shared_data.get("records", [])
    for raw in records:
        assert len(raw) >= 2, "RECORD too short: {}".format(raw.hex())
        from provisa.bolt.messages import RECORD

        assert raw[1] == RECORD, "Expected RECORD tag 0x71, got 0x{:02X}".format(raw[1])
        list_byte = raw[2] if len(raw) > 2 else 0x90
        assert (list_byte & 0xF0) == 0x90 or list_byte in (0xD4, 0xD5, 0xD6), (
            "RECORD field is not a PackStream list: 0x{:02X}".format(list_byte)
        )
...
def step_response_serialized_packstream_framed(shared_data: dict) -> None:
    ...
    for key in ("hello_response", "run_response", "pull_final"):
        ...
        if not raw:
            continue
        assert len(raw) >= 2, "{} too short to be a valid PackStream message".format(key)
        assert (raw[0] & 0xF0) == 0xB0, (
            "{} does not start with a PackStream tiny-struct header: 0x{:02X}".format(key, raw[0])
        )
        if len(raw) > 2:
            try:
                value = unpack(raw[2:])
                assert isinstance(value, (dict, list)), "{} decoded to unexpected type {}".format(
                    key, type(value)
                )
            except Exception as exc:
                pytest.fail("PackStream decode failed for {}: {}".format(key, exc))
...
def test_packstream_node_struct_bytes():
    ...
    data = pack(node)
    assert data[0] == 0xB4, "Expected 0xB4, got 0x{:02X}".format(data[0])
    assert data[1] == 0x4E, "Expected TAG_NODE 0x4E, got 0x{:02X}".format(data[1])
...
def test_packstream_relationship_struct_bytes():
    ...
    data = pack(rel)
    assert data[0] == 0xB8, "Expected 0xB8, got 0x{:02X}".format(data[0])
    assert data[1] == 0x52, "Expected TAG_RELATIONSHIP 0x52, got 0x{:02X}".format(data[1])
...
def test_bolt_session_hello_bolt5():
    """BoltSession.handle_hello for Bolt 5.x transitions to AUTHENTICATION."""
...
def test_packstream_scalars_roundtrip():
    ...
    for value in cases:
        ...
        if isinstance(value, float):
            assert abs(decoded - value) < 1e-9, "float roundtrip failed for {}".format(value)
        else:
            assert decoded == value, "Roundtrip failed for {!r}: got {!r}".format(value, decoded)
...
def test_version_negotiation_logic():
    """Simulate the server-side version negotiation logic."""
    from provisa.bolt.messages import SUPPORTED_VERSIONS

    def _candidates(b4: bytes) -> list:
...
def test_encode_version_proposal_structure():
    ...
    assert proposal[0] == 0x00
    assert proposal[1] == 0x00
    assert proposal[2] == 4
    assert proposal[3] == 5
...
def test_make_chunked_framing():
    """_make_chunked must produce a valid Bolt chunk."""
    payload = b"\xb1\x70\xa0"
...
def test_session_handle_logon_transitions_to_ready():
    ...
    fw = _FakeWriter()
    session = BoltSession(fw, bolt_version=(5, 4))
    session._resolve_user = lambda principal, credentials: (principal, [principal])
...
def test_session_handle_logon_bad_credentials_stays_authentication():
    """LOGON with bad credentials must send FAILURE."""
    from provisa.bolt.session import BoltSession, State
    from provisa.bolt.messages import FAILURE
    ...
    fw = _FakeWriter()
    session = BoltSession(fw, bolt_version=(5, 4))
    session._resolve_user = lambda principal, credentials: None
...
# Copyright (c) 2026 Kenneth Stott
# Canary: 715fc797-768c-457e-82a8-fd57b4c31902
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: 3d79105b-77a9-446f-a904-7c04485f4cee
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: 5742002f-575a-471e-9636-4bf0ab54c760
#
# This source code is licensed under the Business Source License 1.1
...
# Copyright (c) 2026 Kenneth Stott
# Canary: 181bbda1-738c-4ffa-97e0-82ac638a2f08
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: 3f69ef00-55b7-4e74-b631-788544f7320c
#
# This source code is licensed under the Business Source License 1.1

# All steps for REQ-802 default behaviour scenario are already present in the
# existing steps file.  Nothing new to append.


# Copyright (c) 2026 Kenneth Stott
# Canary: f4bf458b-7eac-4a76-bc68-d2e147b26578
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: 1b3419ad-dd46-4ccf-bb03-678e048d4f48
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: 6537f43a-794b-4e87-be53-bb426c8656d0
#
# This source code is licensed under the Business Source License 1.1
