...
def step_server_accepts_handshake(shared_data: dict) -> None:
    ...
    chosen = shared_data.get("chosen_version", b"\x00\x00\x00\x00")
    assert chosen != b"\x00\x00\x00\x00", (
        "Server rejected all version proposals; got {}".format(chosen.hex())
    )
    from provisa.bolt.messages import SUPPORTED_VERSIONS

    major, minor = chosen[3], chosen[2]
    assert (major, minor) in SUPPORTED_VERSIONS, (
        "Server chose unsupported version ({}, {})".format(major, minor)
    )
...
def step_query_transpiled_to_sql(shared_data: dict) -> None:
    ...
    tag = run_response[1]
    assert tag != FAILURE, (
        "Server returned FAILURE for RUN; transpilation failed. raw={}".format(
            run_response.hex()
        )
    )
...
def step_response_serialized_packstream_framed(shared_data: dict) -> None:
    ...
    for key in ("hello_response", "run_response", "pull_final"):
        ...
        assert (raw[0] & 0xF0) == 0xB0, (
            "{} does not start with a PackStream tiny-struct header: 0x{:02X}".format(
                key, raw[0]
            )
        )
        if len(raw) > 2:
            try:
                value = unpack(raw[2:])
                assert isinstance(value, (dict, list)), (
                    "{} decoded to unexpected type {}".format(key, type(value))
                )
            except Exception as exc:
...
# Copyright (c) 2026 Kenneth Stott
# Canary: cd06884a-3c06-4e1e-9a0f-6ba43e306cf6
#
# This source code is licensed under the Business Source License 1.1


# Copyright (c) 2026 Kenneth Stott
# Canary: e96a2fe3-a78f-4d25-9810-50ad6777ab67
#
# This source code is licensed under the Business Source License 1.1
