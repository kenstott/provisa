...
# Copyright (c) 2026 Kenneth Stott
# Canary: de1b9b88-8268-4243-94a9-7d39e8b6dd07
#
# This source code is licensed under the Business Source License 1.1
...
def then_validation_passes_and_routes_to_debezium(shared_data):
    ...
    state = SimpleNamespace(cdc_sources={source.id: source})
    provider = _resolve_provider_type(source.type.value, source.id, _tbl_meta("debezium"), state)
    assert provider == "debezium", "Expected 'debezium', got {!r}".format(provider)
...
def test_debezium_capable_sources_route_to_debezium_via_strategy(stype):
    ...
    provider = _resolve_provider_type(src.type.value, src.id, _tbl_meta("debezium"), state)
    assert provider == "debezium", (
        "Expected 'debezium' for {} with strategy=debezium, got {!r}".format(stype, provider)
    )
...
# No new steps, imports, or scenario registrations are needed.
# All steps for REQ-824 are already fully implemented in the existing file.


# No new steps, imports, or scenario registrations are needed.
# All steps for REQ-824 are already fully implemented in the existing file content.
