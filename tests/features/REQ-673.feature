...
Feature: REQ-673 — Source Connectors
  # GQL remote sources support an optional per-source config field (e.g. `count_query`) for retrieving node counts directly from the remote GraphQL API when the local Trino cache is cold.
