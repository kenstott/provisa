...
async def test_hive_registers_and_serves_semantic_query(connector_client):  # noqa: F811
    ...
    try:
        ...
        # Two Hive-connector catalogs (this one and hive_s3_pipeline) coexisting in the same
        # Trino coordinator triggers a Hive-connector metastore-client cross-catalog contamination
        # bug when both run in the same e2e-lane session — drop this catalog as soon as the test
        # is done so at most one Hive-connector catalog is ever live at a time.
