...
async def me(request: Request):
    ...
    if identity is None or identity.user_id == "anonymous":
        ...
        return {
            ...
            "assignments": [
                {"role_id": rid, "domain_id": "*"} for rid in sorted(all_role_ids)
            ],
        }
...
async def provider_type():
    ...
    provider = auth_cfg.get("provider") if isinstance(auth_cfg, dict) else getattr(auth_cfg, "provider", None)
...
async def register(body: RegisterRequest):
    ...
    if provider != "basic":
        from fastapi import HTTPException
        raise HTTPException(status_code=400, detail="Registration only available with basic auth provider")
    ...
    async with pg_pool.acquire() as conn:
        ...
        await conn.execute(
            ...
            user_id, body.username, password_hash, body.email, body.display_name,
        )
