...
CREATE INDEX IF NOT EXISTS idx_tenant_config_lookup ON tenant_config (tenant_id, entity_type);
"""
...
async def create_tenant(pool: asyncpg.Pool, kms_key_arn: str) -> Tenant:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO tenants (kms_key_arn)
            VALUES ($1)
...
async def update_tenant_plan(
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE tenants SET plan = $1, source_limit = $2 WHERE id = $3",
            plan,
            source_limit,
            uuid.UUID(tenant_id),
        )
...
async def upsert_config_entity(
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO tenant_config (tenant_id, entity_type, entity_id, encrypted_dek, ciphertext, iv)
            VALUES ($1, $2, $3, $4, $5, $6)
