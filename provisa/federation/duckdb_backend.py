...
class DuckDBBackend(EngineBackend):
    ...
    def isolated_sync(self, state: Any):
        ...
        store attached. The engine cache writes CREATE TABLE/INSERT against ``mat_store.*`` land in
        the store (not DuckDB's own storage). A missing store errors at attach (the engine invariant)."""
    ...
    def _materialize_store_ref(self, state: Any) -> str | None:
        ...
        reach live are cached in the materialization store, attached under its alias — never DuckDB's
        own storage. A missing store is a hard error (raised by the runtime)."""
