...
test.describe("LoginPage", () => {
  ...
  // ── Auth disabled mode (works in any environment) ──────────────────────────
  ...
  // ── Logout (works in any environment) ──────────────────────────────────────
  ...
  // ── Form-dependent tests (skipped when auth is disabled) ───────────────────

  test.describe("with auth form", () => {
    test.beforeEach(async ({ page }, testInfo) => {
      const formVisible = await page.locator("form").isVisible({ timeout: 3000 }).catch(() => false);
      testInfo.skip(!formVisible, "Auth disabled (VITE_AUTH_ENABLED != true) — no login form");
    });
