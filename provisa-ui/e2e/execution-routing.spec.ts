...
import { test, expect, BACKEND_URL, FLIGHT_PORT } from "./coverage";
...
test("single-source query routes to direct RDBMS", async ({ request }) => {
  const resp = await request.post(`${BACKEND_URL}/data/graphql`, {
    data: {
      query: `query {
        __typename
      }`,
...
test("single-source query transpiles via SQLGlot to target dialect", async ({
  ...
  expect(body.compiled[0].route).toBeDefined();
});
...
test("cross-source query routes to Trino", async ({ request }) => {
  ...
  expect(["direct", "virtual"]).toContain(body.compiled[0].route);
});
...
test("Arrow Flight server responds on port 8815 (gRPC endpoint)", async () => {
  // Arrow Flight/gRPC is a binary HTTP/2 protocol — an HTTP/1.1 fetch()
  // fails to parse its frames. Verify TCP-level reachability instead;
  // actual gRPC protocol testing would require a gRPC client library.
  await new Promise<void>((resolve, reject) => {
    const socket = net.connect(Number(FLIGHT_PORT), "localhost", () => {
      socket.end();
      resolve();
    });
...
test("routing decision endpoint exposes route selection reasoning", async ({
  ...
  expect(["direct", "virtual", "api"]).toContain(body.compiled[0].route);
});
