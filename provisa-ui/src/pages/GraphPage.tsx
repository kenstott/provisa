    ...
    >
      ...
      <div className="node-ctx-size-row">
        <input
          ...
          value={current.width}
          onChange={(e) =>
            onRelLineChange(menu.type, { ...current, width: Number(e.target.value) })
          }
        />
...
export function GraphPage() {
  ...
  const frameIdRef = useRef(
    _graphState.frames.reduce((max, f) => Math.max(max, parseInt(f.id) || 0), 0),
  );
