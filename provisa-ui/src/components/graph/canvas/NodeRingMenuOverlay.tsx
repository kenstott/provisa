...
export function NodeRingMenuOverlay({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const rp = (
        cyNode as unknown as { renderedPosition: () => { x: number; y: number } }
      ).renderedPosition();
  ...
  const arcSector = (centerDeg: number) => {
    ...
    const ox1 = R2 * Math.cos(a1),
      oy1 = R2 * Math.sin(a1);
    const ox2 = R2 * Math.cos(a2),
      oy2 = R2 * Math.sin(a2);
    const ix1 = R1 * Math.cos(a2),
      iy1 = R1 * Math.sin(a2);
    const ix2 = R1 * Math.cos(a1),
      iy2 = R1 * Math.sin(a1);
  ...
  const sectors: {
  ...
  }[] = [
    {
      ...
      iconPath: (
        <>
          ...
          <path
            d="M-2.5 0 v-2.5 a2.5 2.5 0 0 1 5 0"
            fill="none"
            stroke="currentColor"
            strokeWidth="1.1"
            strokeLinecap="round"
    ...
    },
    {
      angle: 30,
      key: "children",
      active: showingChildrenNatural.has(nodeRingMenu.nodeKey),
    ...
    },
    {
      ...
      iconPath: (
        <>
          {/* eye */}
          <path
            d="M-5 0 C-3.5-3 3.5-3 5 0 C3.5 3-3.5 3-5 0 Z"
            fill="none"
            stroke="currentColor"
            strokeWidth="0.9"
          />
