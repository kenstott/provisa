...
import { injectExclusion } from "./graph-model";
...
import { useFrameDerivedData } from "./frame/use-frame-derived-data";
import { useAutoImpute } from "./frame/use-auto-impute";
import {
  GraphViewIcon,
  TableViewIcon,
  JsonViewIcon,
...
export function GraphFrame({
  ...
  } = useFrameDerivedData(frame, overlayData);
  ...
  useAutoImpute(frame, autoImpute, setOverlayData);
  ...
  const groupableAttrs = useMemo(() => {
    ...
    const regularAttrs = [...counts.entries()].filter(([, vals]) => vals.size > 1).map(([k]) => k);
    return ["domain", ...schemaVirtuals, ...degreeVirtuals, ...regularAttrs].sort((a, b) =>
      a.localeCompare(b),
    );
  }, [augmentedNodes]);
        ...
        <div className="gf-header-actions">
          ...
          {onAddFavorite && (
            <ActionIcon
              ...
              title={
                isFavorited?.(editQuery.trim())
                  ? t("graphFrame.removeFromFavorites")
                  : t("graphFrame.addToFavorites")
              }
              aria-label={
                isFavorited?.(editQuery.trim())
                  ? t("graphFrame.removeFromFavorites")
    ...
    <div className="gf-body" style={expanded ? undefined : { height: graphAreaHeight }}>
      <div className="gf-view-bar">
        {[
          ...
          {
            key: "json" as const,
            label: t("graphFrame.viewJson"),
            Icon: JsonViewIcon,
            show: true,
            onClick: () => setView("json"),
          },
          {
            key: "graphstats" as const,
            label: t("graphFrame.viewGraphStats"),
            Icon: StatsViewIcon,
            show: hasGraph,
          ...
          {
            key: "code" as const,
            label: t("graphFrame.viewCode"),
            Icon: CodeViewIcon,
            show: true,
            onClick: () => setView("code"),
          },
        ].map(
          ({ key, label, Icon, show, onClick }) =>
            show && (
              <ActionIcon
                key={key}
                variant="subtle"
      ...
      <div className="gf-body-content">
        ...
        {frame.status !== "error" && hasGraph && (
          <div
            ...
            style={{
              flexBasis: graphAreaHeight,
              display: activeView === "graph" ? undefined : "none",
            }}
          >
            <GraphCanvas
        ...
        {frame.status !== "error" &&
          ...
          (() => {
            ...
            const stats = frame.queryStats as
              | { total_elapsed_ms?: number; sources?: SourceEntry[]; mermaid?: string }
              | undefined;
              ...
              ...(stats
                ? {
                    stats: {
                      ...
                      sources: (stats.sources ?? []).map(({ physical_sql: _, ...rest }) => rest),
                    },
                  }
                : {}),
            };
    ...
    <>
      <div
        ...
        style={
          expanded
            ? {
                top: `calc(2.5vh + ${modalHeaderHeight}px)`,
                height: `calc(95vh - ${modalHeaderHeight}px)`,
              }
            : undefined
        }
        onDragOver={(e) => {
