...
/**
 * Group fields by domain prefix (splitting on "__") and render
 * them inside collapsible <details> elements. Uses native HTML
...
function _renderGroupedFields(fields, renderField, selections) {
  ...
  fieldNames.forEach(function (fieldName) {
    var sepIdx = fieldName.indexOf('__');
    if (sepIdx > 0) {
      var domain = fieldName.slice(0, sepIdx);
      if (!domains[domain]) domains[domain] = [];
      domains[domain].push(fieldName);
    } else {
      ungrouped.push(fieldName);
    }
  });

  var domainKeys = Object.keys(domains).sort();

  // If no domains found, render flat (original behavior)
    ...
    result.push(
      React.createElement(
        ...
        React.createElement(
          'summary',
          {
            className: 'graphiql-explorer-domain-summary',
          ...
          React.createElement(
            'span',
            {
              className: 'graphiql-explorer-domain-arrow',
              style: {
                display: 'inline-block',
                width: '8px',
                fontSize: '8px',
        ...
        React.createElement(
          'div',
          { style: { paddingLeft: 8 } },
          domains[domain].map(renderField)
        )
      )
    );
  });

  // Render ungrouped fields at the end
  ungrouped.forEach(function (fieldName) {
    result.push(renderField(fieldName));
  });
  ...
  _createClass(RootView, [{
    ...
    value: function render() {
      ...
      return React.createElement(
        ...
        _renderGroupedFields(fields, function (fieldName) {
          return React.createElement(FieldView, {
            key: fieldName,
            field: fields[fieldName],
            selections: selections,
...
exports.Explorer = ExplorerWrapper;
exports._renderGroupedFields = _renderGroupedFields;