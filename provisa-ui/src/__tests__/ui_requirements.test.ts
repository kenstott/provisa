...
import { clusterColor, extractElements, injectExclusion } from "../components/graph/graph-model";
...
describe("REQ-648: collapse-to-supernode produces __collapsed_<level>_<id> node", () => {
  ...
  it("REQ-648: cross-cluster meta-edge targets supernode when cluster is collapsed", () => {
    ...
    expect(metaEdge?.data.source === collapsedId || metaEdge?.data.target === collapsedId).toBe(
      true,
    );
  });
