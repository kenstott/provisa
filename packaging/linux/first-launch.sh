...
# ── TLS certificates (all protocol endpoints, REQ-1227) ───────────────────────
...
write_node_overlay() {
  ...
  mkdir -p "$dir"
  cat > "$file" <<YAML
# Auto-generated by first-launch.sh — REQ-1227: TLS on every endpoint + cluster role.
services:
  provisa:
...
# intra-cluster network at ${PRIMARY_IP}. REQ-1227: all endpoints served over TLS.
services:
  provisa:
