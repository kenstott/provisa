...
write_demo_overlay() {
  ...
  # Only the primary loads config from a file; secondaries pull the shared config
  # (sources, policies, semantic model) from the primary's PostgreSQL at runtime, so
  # pinning them to the demo file would bypass the cluster's source of truth.
  if [ "$(_lc "${INSTALL_DEMO:-n}")" != "y" ] || [ "$ROLE" != "primary" ]; then
    rm -f "$file" 2>/dev/null || true
    return
  fi
