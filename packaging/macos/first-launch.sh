...
# version records the release that wrote this config; the launcher compares it to
# the bundle's Resources/VERSION and offers to overwrite a config from an earlier
# release (whose shape may predate keys the current CLI requires, e.g. runtime).
version: ${PROVISA_VERSION:-dev}
...
#   PROVISA_DQ_CHECKER   none (default) | soda | gx — REQ-1443's external data-quality checker,
#                        installed as a pyproject extra only when the operator names one
# Sets globals: DEPLOY_ENGINE OBS_MODE INSTALL_DEMO DEMO_MODE NEEDS_DOCKER DQ_CHECKER
resolve_deployment() {
